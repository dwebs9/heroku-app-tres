...
def register():
    ...
    if form.validate_on_submit():
        ...
        new_user = User(
            name=name, lastName=lastName, password_hash=pwd_hash, emailid=email
        )
        db.session.add(new_user)
        db.session.commit()
        print("COMMITED TO DB")
        register_success = 'The Email "{}"successfully registered'.format(email)
